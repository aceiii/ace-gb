...
int run_cpu_tests() {
  ...
  for (int n = 0; n < 256; n++) {
    std::filesystem::path path(
        fmt::format("../external/sm83/v1/{:02x}.json", n));
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      spdlog::info("Running test #{} of {}: {}", i, data.size(), name);
      ...
      auto is_success = a_match && b_match && c_match && d_match && e_match &&
                        f_match && h_match && l_match && sp_match && pc_match &&
                        ram_match;
      if (is_success) {
        success += 1;
  ...
  return total_success_suites == total_test_suites ? 0 : 1;
}
